...
if [ $# -eq 0 ]; then
	...
	echo -e "\t--length_min <integer>\tTarget block length (in units of centimorgans) [Default = 1]"
	echo -e "\t--width <integer>\tNumber of haplotypes in block [Default = 20]"
	echo -e "\t--width_min <integer>\tTarget block width [Default = 10]"
...
LONGOPTS=checkpoint:,inputVCF:,writeTo:,length:,length_min:,width:,width_min:,gap:,map:,rho:

PARSED=$(getopt --options=$OPTIONS --longoptions=$LONGOPTS --name "$0" -- "$@")
eval set -- "$PARSED" 

checkpoint=100000 inputVCF="" writeTo="" length=20 length_min=1 width=20 width_min=10 gap=1 map="" rho=0.05
while true; do
	case "$1" in
		-c|--checkpoint)
			checkpoint="$2"
...
echo "Running PBWT..."
./PBWT "$inputVCF" "$writeTo" "$map" "$checkpoint" "$length_min" "$length" "$width_min" "$width" "$gap" "$rho"
rm "${writeTo}.rpbwt" "${writeTo}.sites" "${writeTo}.meta"
echo "PS-cluster Finished."
